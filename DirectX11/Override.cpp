...
void Override::ParseIniSection(LPCWSTR section)
{
	...
	transition_type =
	    GetIniEnumClass(section, L"transition_type", TransitionType::LINEAR, nullptr, TransitionTypeNames);
	release_transition_type =
	    GetIniEnumClass(section, L"release_transition_type", TransitionType::LINEAR, nullptr, TransitionTypeNames);
	...
	if (GetIniStringAndLog(section, L"run", nullptr, buf, MAX_PATH))
	{
		...
		if (!ParseRunExplicitCommandList(section, L"run", &sbuf, nullptr, &activate_command_list,
		                                 &deactivate_command_list, &ini_namespace))
...
struct KeyOverrideCycleParam
{
	...
	template <class T1, class T2> T2 as_enum(EnumName_t<T1, T2> *enum_names, T2 fallback)
	{
		T2 val;
	...
	void as_run_command(LPCWSTR section, CommandList *pre_command_list, CommandList *deactivate_command_list)
	{
		...
		if (!ParseRunExplicitCommandList(section, L"run", &scur, nullptr, pre_command_list, deactivate_command_list,
		                                 &ini_namespace))
...
void KeyOverrideCycle::ParseIniSection(LPCWSTR section)
{
	...
	for (i = 1; not_done; i++)
	{
		...
		presets.push_back(KeyOverride(
		    KeyOverrideType::CYCLE, &params, &vars, transition.as_int(0), release_transition.as_int(0),
...
void Override::Activate(HackerDevice *device, bool override_has_deactivate_condition)
{
	...
	CurrentTransition.ScheduleTransition(device, &mOverrideParams, &mOverrideVars, transition, transition_type);
...
void Override::Deactivate(HackerDevice *device)
{
	...
	CurrentTransition.ScheduleTransition(device, &mSavedParams, &mSavedVars, release_transition,
	                                     release_transition_type);
...
static void _ScheduleTransition(struct OverrideTransitionParam *transition, char *name, float current, float val,
                                ULONGLONG now, int time, TransitionType transition_type)
{
...
// Known limitation: Clean up the wide vs sensible string mess and remove this duplicate function:
static void _ScheduleTransition(struct OverrideTransitionParam *transition, const wchar_t *name, float current,
                                float val, ULONGLONG now, int time, TransitionType transition_type)
{
	LogInfoNoNL(" %S: %#.2g -> %#.2g", name, current, val);
	transition->start = current;
	transition->target = val;
...
void OverrideTransition::ScheduleTransition(HackerDevice *wrapper [[maybe_unused]], OverrideParams *targets,
                                            OverrideVars *var_targets, int time, TransitionType transition_type)
{
	...
	for (i = targets->begin(); i != targets->end(); i++)
	{
		...
		_ScheduleTransition(&params[i->first], buf, G->iniParams[i->first.idx].*i->first.component, i->second, now,
		                    time, transition_type);
	}
	for (j = var_targets->begin(); j != var_targets->end(); j++)
	{
		_ScheduleTransition(&vars[j->first], j->first->name.c_str(), j->first->fval, j->second, now, time,
		                    transition_type);
	}
	LogInfo("\n");
}
...
OverrideGlobalSaveParam::OverrideGlobalSaveParam() : save(FLT_MAX), refcount(0) {}
