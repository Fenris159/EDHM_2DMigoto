...
static void update_txt_summary(LARGE_INTEGER collection_duration, LARGE_INTEGER freq, unsigned frames)
{
	...
	shader_hash_lookup_overhead.QuadPart =
	    Profiling::shader_hash_lookup_overhead.cpu.QuadPart * 1000000 / freq.QuadPart;
	shader_reload_lookup_overhead.QuadPart =
	    Profiling::shader_reload_lookup_overhead.cpu.QuadPart * 1000000 / freq.QuadPart;
	shader_original_lookup_overhead.QuadPart =
	    Profiling::shader_original_lookup_overhead.cpu.QuadPart * 1000000 / freq.QuadPart;
	shaderoverride_lookup_overhead.QuadPart =
	    Profiling::shaderoverride_lookup_overhead.cpu.QuadPart * 1000000 / freq.QuadPart;
	texture_handle_info_lookup_overhead.QuadPart =
	    Profiling::texture_handle_info_lookup_overhead.cpu.QuadPart * 1000000 / freq.QuadPart;
	textureoverride_lookup_overhead.QuadPart =
	    Profiling::textureoverride_lookup_overhead.cpu.QuadPart * 1000000 / freq.QuadPart;
	resource_pool_lookup_overhead.QuadPart =
	    Profiling::resource_pool_lookup_overhead.cpu.QuadPart * 1000000 / freq.QuadPart;

	Profiling::text += L" (CPU Performance Summary):\n";
	_snwprintf_s(
	    buf, ARRAYSIZE(buf), _TRUNCATE,
	    ...
	    (float)present_overhead.QuadPart / frames, 60.0 * present_overhead.QuadPart / collection_duration.QuadPart,

	    (float)overlay_overhead.QuadPart / frames, 60.0 * overlay_overhead.QuadPart / collection_duration.QuadPart,

	    (float)draw_overhead.QuadPart / frames, 60.0 * draw_overhead.QuadPart / collection_duration.QuadPart,

	    (float)command_list_overhead.QuadPart / frames,
	    60.0 * command_list_overhead.QuadPart / collection_duration.QuadPart,

	    (float)map_overhead.QuadPart / frames, 60.0 * map_overhead.QuadPart / collection_duration.QuadPart,
	    ...
	    (float)stat_overhead.QuadPart / frames, 60.0 * stat_overhead.QuadPart / collection_duration.QuadPart,

	    (float)shaderregex_overhead.QuadPart / frames,
	    60.0 * shaderregex_overhead.QuadPart / collection_duration.QuadPart,

	    (float)cursor_overhead.QuadPart / frames, 60.0 * cursor_overhead.QuadPart / collection_duration.QuadPart);
	Profiling::text += buf;
	...
	_snwprintf_s(
	    ...
	    Profiling::shader_hash_lookup_overhead.hits / frames, Profiling::shader_hash_lookup_overhead.count / frames,
	    ...
	    Profiling::shader_reload_lookup_overhead.hits / frames, Profiling::shader_reload_lookup_overhead.count / frames,
	...
	_snwprintf_s(buf, ARRAYSIZE(buf), _TRUNCATE,
	             ...
	             Profiling::resource_full_copies / frames, Profiling::resource_reference_copies / frames,
	             Profiling::inter_device_copies / frames, Profiling::msaa_resolutions / frames,
	             Profiling::buffer_region_copies / frames, Profiling::views_cleared / frames,
	             Profiling::resources_created, Profiling::resource_pool_swaps / frames,
	             Profiling::max_copies_per_frame_exceeded / frames, Profiling::injected_draw_calls / frames,
	             Profiling::skipped_draw_calls / frames, Profiling::max_executions_per_frame_exceeded / frames);
	Profiling::text += buf;
...
{
	LARGE_INTEGER inclusive;
	LARGE_INTEGER exclusive;
	double inclusive_fps;
	double exclusive_fps;
	wchar_t buf[256];
	...
	for (CommandList *command_list : sorted)
	{
		...
		_snwprintf_s(buf, ARRAYSIZE(buf), _TRUNCATE, L"%5.0f | %7.2fus %9f | %7.2fus %9f | %4s [%s]\n",
		             ceil((float)command_list->executions / frames), (float)inclusive.QuadPart / frames, inclusive_fps,
		             (float)exclusive.QuadPart / frames, exclusive_fps, command_list->post ? L"post" : L"pre",
...
static void update_txt_commands(LARGE_INTEGER collection_duration, LARGE_INTEGER freq, unsigned frames)
{
	...
	for (CommandListCommand *cmd : sorted)
	{
		...
		{
			_snwprintf_s(buf, ARRAYSIZE(buf), _TRUNCATE, L"%5.0f %7.2fus %9f | ",
			             ceil((float)cmd->pre_executions / frames), (float)pre_time_spent.QuadPart / frames,
			             pre_fps_cost);
			Profiling::text += buf;
		}
		else
		...
		{
			_snwprintf_s(buf, ARRAYSIZE(buf), _TRUNCATE, L"%5.0f %7.2fus %9f ",
			             ceil((float)cmd->post_executions / frames), (float)post_time_spent.QuadPart / frames,
			             post_fps_cost);
			Profiling::text += buf;
		}
		else
...
void Profiling::update_txt()
{
	...
	LARGE_INTEGER collection_duration;
	...
	{
		_snwprintf_s(buf, ARRAYSIZE(buf), _TRUNCATE, L"Performance Monitor %.1ffps",
		             frames * 1000000.0 / collection_duration.QuadPart);
		Profiling::text = buf;
