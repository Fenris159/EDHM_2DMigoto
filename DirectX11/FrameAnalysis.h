...
DEFINE_GUID(IID_FrameAnalysisContext, 0x2aee5b3a, 0x68ed, 0x44e9, 0xaa, 0x4d, 0x9e, 0xaa, 0x63, 0x15, 0xd7, 0x2b);

// {4EAF92BD-5552-44C7-804C-4CE1014C1B17}
DEFINE_GUID(InputLayoutDescGuid, 0x4eaf92bd, 0x5552, 0x44c7, 0x80, 0x4c, 0x4c, 0xe1, 0x1, 0x4c, 0x1b, 0x17);
...
struct FrameAnalysisDeferredDumpBufferArgs
{
	...
	FrameAnalysisDeferredDumpBufferArgs(FrameAnalysisOptions analyse_options, ID3D11Buffer *staging,
	    ...
	    : analyse_options(analyse_options), staging(staging), orig_desc(*orig_desc), filename(filename),
	      buf_type_mask(buf_type_mask), idx(idx), ib_fmt(ib_fmt), stride(stride), offset(offset), first(first),
	      count(count), layout(layout), topology(topology), call_info(call_info ? *call_info : DrawCallInfo()),
...
struct FrameAnalysisDeferredDumpTex2DArgs
{
	...
	FrameAnalysisDeferredDumpTex2DArgs(FrameAnalysisOptions analyse_options, ID3D11Texture2D *staging,
	                                   wchar_t *filename, D3D11_TEXTURE2D_DESC *orig_desc, DXGI_FORMAT format)
	    : analyse_options(analyse_options), staging(staging), filename(filename), orig_desc(*orig_desc), format(format)
	{
	}
};
...
{
  private:
	template <class ID3D11Shader> void FrameAnalysisLogShaderHash(ID3D11Shader *shader);
	...
	void FrameAnalysisLogConstantBuffer(int slot, char *slot_name, ID3D11Resource *resource, UINT first_constant,
	                                    UINT num_constants);
	void FrameAnalysisLogConstantBufferArray(UINT start, UINT len, ID3D11Resource *const *ppResources,
	...
	void Dump2DResource(ID3D11Texture2D *resource, wchar_t *filename, D3D11_TEXTURE2D_DESC *orig_desc,
	                    DXGI_FORMAT format);
	bool DeferDump2DResource(ID3D11Texture2D *staging, wchar_t *filename, D3D11_TEXTURE2D_DESC *orig_desc,
	                         DXGI_FORMAT format);
	void Dump2DResourceImmediateCtx(ID3D11Texture2D *staging, wstring filename, D3D11_TEXTURE2D_DESC *orig_desc,
	                                DXGI_FORMAT format);

	HRESULT ResolveMSAA(ID3D11Texture2D *src, D3D11_TEXTURE2D_DESC *srcDesc, ID3D11Texture2D **resolved,
	                    DXGI_FORMAT format);
	HRESULT StageResource(ID3D11Texture2D *src, D3D11_TEXTURE2D_DESC *srcDesc, ID3D11Texture2D **dst,
	                      DXGI_FORMAT format);
	HRESULT CreateStagingResource(ID3D11Texture2D **resource, D3D11_TEXTURE2D_DESC desc, bool msaa, DXGI_FORMAT format);

	void DumpBufferTxt(wchar_t *filename, D3D11_MAPPED_SUBRESOURCE *map, UINT size, char type, int idx, UINT stride,
	                   UINT offset);
	void DumpVBTxt(wchar_t *filename, D3D11_MAPPED_SUBRESOURCE *map, UINT size, int idx, UINT stride, UINT offset,
	               UINT first, UINT count, HackerInputLayout *layout, D3D11_PRIMITIVE_TOPOLOGY topology,
	               DrawCallInfo *call_info);
	void DumpIBTxt(wchar_t *filename, D3D11_MAPPED_SUBRESOURCE *map, UINT size, DXGI_FORMAT ib_fmt, UINT offset,
	               UINT first, UINT count, D3D11_PRIMITIVE_TOPOLOGY topology);

	void DumpBuffer(ID3D11Buffer *buffer, wchar_t *filename, FrameAnalysisOptions buf_type_mask, int idx,
	                DXGI_FORMAT ib_fmt, UINT stride, UINT offset, UINT first, UINT count, HackerInputLayout *layout,
	                D3D11_PRIMITIVE_TOPOLOGY topology, DrawCallInfo *call_info, ID3D11Buffer **staged_ib_ret,
	                ID3D11Buffer *staged_ib_for_vb, UINT ib_off_for_vb);
	bool DeferDumpBuffer(ID3D11Buffer *staging, D3D11_BUFFER_DESC *orig_desc, wchar_t *filename,
	                     FrameAnalysisOptions buf_type_mask, int idx, DXGI_FORMAT ib_fmt, UINT stride, UINT offset,
	                     UINT first, UINT count, HackerInputLayout *layout, D3D11_PRIMITIVE_TOPOLOGY topology,
	                     DrawCallInfo *call_info, ID3D11Buffer *staged_ib_for_vb, UINT ib_off_for_vb);
	void DumpBufferImmediateCtx(ID3D11Buffer *staging, D3D11_BUFFER_DESC *orig_desc, wstring filename,
	                            FrameAnalysisOptions buf_type_mask, int idx, DXGI_FORMAT ib_fmt, UINT stride,
	                            UINT offset, UINT first, UINT count, HackerInputLayout *layout,
	                            D3D11_PRIMITIVE_TOPOLOGY topology, DrawCallInfo *call_info,
	                            ID3D11Buffer *staged_ib_for_vb, UINT ib_off_for_vb);

	void DumpResource(ID3D11Resource *resource, wchar_t *filename, FrameAnalysisOptions buf_type_mask, int idx,
	...
	template <typename DescType> void DumpDesc(DescType *desc, const wchar_t *filename);

	void dump_deferred_resources(ID3D11CommandList *command_list);
	void finish_deferred_resources(ID3D11CommandList *command_list);

	HRESULT FrameAnalysisFilename(wchar_t *filename, size_t size, bool compute, const wchar_t *reg, char shader_type,
	                              int idx, ID3D11Resource *handle, uint32_t override_hash = 0);
	HRESULT FrameAnalysisFilenameResource(wchar_t *filename, size_t size, const wchar_t *type, ID3D11Resource *handle,
	                                      bool force_filename_handle);
	const wchar_t *dedupe_tex2d_filename(ID3D11Texture2D *resource, D3D11_TEXTURE2D_DESC *desc,
	                                     wchar_t *dedupe_filename, size_t size, const wchar_t *traditional_filename,
	                                     DXGI_FORMAT format);
	void dedupe_buf_filename(ID3D11Buffer *resource, D3D11_BUFFER_DESC *orig_desc, D3D11_MAPPED_SUBRESOURCE *map,
	                         wchar_t *dedupe_filename, size_t size);
	void dedupe_buf_filename_txt(const wchar_t *bin_filename, wchar_t *txt_filename, size_t size, char type, int idx,
	                             UINT stride, UINT offset);
	void dedupe_buf_filename_vb_txt(const wchar_t *bin_filename, wchar_t *txt_filename, size_t size, int idx,
	                                UINT stride, UINT offset, UINT first, UINT count, HackerInputLayout *layout,
	                                D3D11_PRIMITIVE_TOPOLOGY topology, DrawCallInfo *call_info);
	void dedupe_buf_filename_ib_txt(const wchar_t *bin_filename, wchar_t *txt_filename, size_t size, DXGI_FORMAT ib_fmt,
	...
	void determine_vb_count(UINT *count, ID3D11Buffer *staged_ib_for_vb, DrawCallInfo *call_info, UINT ib_off_for_vb,
	                        DXGI_FORMAT ib_fmt);
	...
	void _FrameAnalysisAfterUpdate(ID3D11Resource *pResource, FrameAnalysisOptions type_mask, const wchar_t *type);
...
#define FrameAnalysisLogNoNL FrameAnalysisLog
	...
	void FrameAnalysisDump(ID3D11Resource *resource, FrameAnalysisOptions options, const wchar_t *target,
	                       DXGI_FORMAT format, UINT stride, UINT offset) override;
	...
	STDMETHOD(FinishCommandList)(THIS_ BOOL RestoreDeferredContextState,
	                             /* [annotation] */
