...
template <size_t Size> void SanitizeForLog(std::array<char, Size> &value)
{
...
wine_get_version_fn ResolveWineGetVersion()
{
	...
	return reinterpret_cast<wine_get_version_fn>(GetProcAddress(ntdll, "wine_get_version"));
}
...
BOOL CALLBACK InitializeWineDetection(PINIT_ONCE, PVOID, PVOID *)
{
	...
	if (HasEnvironmentVariable("WINEPREFIX") || HasEnvironmentVariable("WINELOADER") ||
...
void LogDllOverrides()
{
	...
	DWORD length = GetEnvironmentVariableA("WINEDLLOVERRIDES", value.data(), static_cast<DWORD>(value.size()));
	if (!length)
	{
...
void LogFlatpakEnvironment()
{
	...
	DWORD length = GetEnvironmentVariableA("FLATPAK_ID", flatpak_id.data(), static_cast<DWORD>(flatpak_id.size()));
	if (!length)
	{
...
bool ApplyWineCompatProfile(int wine_compat_ini, int *load_library_redirect, bool *check_foreground_window)
{
...
void LogHostCompatReport()
{
	...
	DWORD migoto_path_len =
	    GetModuleFileNameW(migoto_handle, migoto_path.data(), static_cast<DWORD>(migoto_path.size()));
	...
	LogInfo("    1) Place the EDHM configuration and 3Dmigoto d3d11.dll next to EliteDangerous64.exe (not only the "
	        "launcher)\n");
