...
static NVStereoSetting NVStereoSettingNames[] = {
    ...
    {0x00ce2692, false, false,
	 L"Texture filtering - Quality substitution"}, // Driver name: "Texture filtering - Quality Substitution"
    {0x0098c1ac, false, false,
	 L"Enable Maxwell sample interleaving (MFAA)"},      // Driver name: "Enable sample interleaving (MFAA)"
    ...
    {0x101ae763, false, false,
	 L"Vertical Sync Smooth AFR behavior"}, // Driver name: "Flag to control smooth AFR behavior"
    {0x00638e8f, false, false,
	 L"Texture filtering - Driver Controlled LOD Bias"}, // Driver name: "Texture filtering - Driver Controlled LOD Bias"
    ...
    {0x10fc2d9c, false, false,
	 L"Antialiasing - Transparency Multisampling"}, // Driver name: "Antialiasing - Transparency Multisampling"
    {0x10d48a85, false, false,
	 L"Antialiasing - Transparency Supersampling"}, // Driver name: "Antialiasing - Transparency Supersampling"
    ...
    {0x0084cd70, false, false,
	 L"Texture filtering - Anisotropic filter optimization"}, // Driver name: "Texture filtering - Anisotropic filter optimization"
    {0x00e73211, false, false,
	 L"Texture filtering - Anisotropic sample optimization"}, // Driver name: "Texture filtering - Anisotropic sample optimization"
    {0x0019bb68, false, false,
	 L"Texture filtering - Negative LOD bias"},                 // Driver name: "Texture filtering - Negative LOD bias"
    {0x00ce2691, false, false, L"Texture filtering - Quality"}, // Driver name: "Texture filtering - Quality"
    {0x002ecaf2, false, false,
	 L"Texture filtering - Trilinear optimization"},        // Driver name: "Texture filtering - Trilinear optimization"
...
typedef NvAPI_Status(__cdecl *tNvAPI_DRS_SaveSettingsToFileEx)(NvDRSSessionHandle hSession,
...
static void _identify_internal_settings(NvDRSSessionHandle session, wchar_t *profile_name,
...
static void identify_internal_settings(NvDRSSessionHandle session, NvDRSProfileHandle profile,
                                       std::unordered_set<unsigned> **internal_settings)
{
	NvAPI_Status status = NVAPI_OK;
...
unsigned char internal_setting_key[] = {
    ,
};
...
unsigned decode_internal_dword(unsigned id, unsigned val)
{
	...
	off = (id << 1);
	key = internal_setting_key[(off + 3) % 256] << 24 | internal_setting_key[(off + 2) % 256] << 16 |
	      internal_setting_key[(off + 1) % 256] << 8 | internal_setting_key[(off + 0) % 256];
...
void decode_internal_string(unsigned id, NvAPI_UnicodeString val)
{
	...
	{
		off = ((id << 1) + i * 2);
		key = internal_setting_key[(off + 1) % 256] << 8 | internal_setting_key[(off + 0) % 256];
...
void _log_nv_profile(NvDRSSessionHandle session, NvDRSProfileHandle profile, NVDRS_PROFILE *info)
{
	...
	LogInfo("Profile \"%S\"%s\n", (wchar_t *)info->profileName, info->isPredefined ? "" : " UserSpecified=true");
	...
	for (i = 0; i < info->numOfSettings; i++)
	{
		...
		internal = settings[i].isCurrentPredefined && settings[i].isPredefinedValid && internal_settings &&
		...
		case NVDRS_WSTRING_TYPE:
			...
			LogInfoNoNL("    SettingString ID_0x%08x = \"%S\"", settings[i].settingId,
...
static int parse_ini_profile_rhs(wstring *rhs, NVDRS_SETTING *setting)
{
	...
	if (val[0] == L'\"' && val[val.length() - 1] == L'\"')
	{
		if (val.length() - 2 < NVAPI_UNICODE_STRING_MAX)
		{
			...
			wcscpy_s((wchar_t *)setting->wszCurrentValue, NVAPI_UNICODE_STRING_MAX,
			         val.substr(1, val.length() - 2).c_str());
			return 0;
		}
	}
...
int parse_ini_profile_line(wstring *lhs, wstring *rhs)
{
	...
	// to ignore:
	if (rhs->empty() && (!_wcsnicmp(lhs->c_str(), L"Profile ", 8) || !_wcsnicmp(lhs->c_str(), L"EndProfile", 10) ||
	                     !_wcsnicmp(lhs->c_str(), L"Executable ", 11) ||
	                     !_wcsnicmp(lhs->c_str(), L"ProfileType ", 12) || !_wcsnicmp(lhs->c_str(), L"ShowOn ", 7)))
	{
...
static bool compare_setting(NvDRSSessionHandle session, NvDRSProfileHandle profile, NVDRS_SETTING *migoto_setting,
                            bool allow_user_customisation, std::unordered_set<unsigned> *internal_settings, bool log)
{
	...
	if (allow_user_customisation && !driver_setting.isCurrentPredefined &&
	...
	internal = driver_setting.isCurrentPredefined && driver_setting.isPredefinedValid && internal_settings &&
