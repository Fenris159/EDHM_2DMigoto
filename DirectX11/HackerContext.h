...
struct DrawContext
{
	...
	DrawContext(DrawCall type, UINT VertexCount, UINT IndexCount, UINT InstanceCount, UINT FirstVertex, UINT FirstIndex,
	            UINT FirstInstance, ID3D11Buffer **indirect_buffer, UINT args_offset)
	    : oldVertexShader(nullptr), oldPixelShader(nullptr),
...
struct DispatchContext
{
	...
	DispatchContext(UINT ThreadGroupCountX, UINT ThreadGroupCountY, UINT ThreadGroupCountZ)
	    : post_commands(nullptr), call_info(DrawCall::Dispatch, 0, 0, 0, 0, 0, 0, nullptr, 0, ThreadGroupCountX,
		                                    ThreadGroupCountY, ThreadGroupCountZ)
	{
	}

	DispatchContext(ID3D11Buffer **indirect_buffer, UINT args_offset)
	    : post_commands(nullptr), call_info(DrawCall::DispatchIndirect, 0, 0, 0, 0, 0, 0, indirect_buffer, args_offset)
	{
	}
};
...
struct MappedResourceInfo
{
	...
	MappedResourceInfo() : orig_pData(nullptr), size(0), mapped_writable(false), bind_flags(0) {}
};
...
class HackerContext : public ID3D11DeviceContext1
{
  private:
	...
	D3D11_PRIMITIVE_TOPOLOGY mCurrentPrimitiveTopology{D3D11_PRIMITIVE_TOPOLOGY_UNDEFINED};
	...
	template <class ID3D11Shader,
	          void (__stdcall ID3D11DeviceContext::*GetShaderVS2013BUGWORKAROUND)(ID3D11Shader **,
	          ...
	          HRESULT (__stdcall ID3D11Device::*CreateShader)(const void *, SIZE_T, ID3D11ClassLinkage *,
	                                                          ID3D11Shader **)>
	...
	bool ExpandRegionCopy(ID3D11Resource *pDstResource, UINT DstX, UINT DstY, ID3D11Resource *pSrcResource,
	                      const D3D11_BOX *pSrcBox, UINT *replaceDstX, D3D11_BOX *replaceBox);
	bool MapDenyCPURead(ID3D11Resource *pResource, UINT Subresource, D3D11_MAP MapType, UINT MapFlags,
	                    D3D11_MAPPED_SUBRESOURCE *pMappedResource);
	bool MapTrackRegionHashes(ID3D11Resource *pResource, D3D11_MAP MapType, D3D11_RESOURCE_DIMENSION *dim);
	void TrackAndDivertMap(HRESULT map_hr, ID3D11Resource *pResource, UINT Subresource, D3D11_MAP MapType,
	...
	template <void (__stdcall ID3D11DeviceContext::*GetShaderResources)(
	    THIS_ UINT StartSlot, UINT NumViews, ID3D11ShaderResourceView **ppShaderResourceViews)>
	...
	void RecordPeerShaders(std::set<UINT64> *PeerShaders, UINT64 this_shader_hash) const;
	...
	SetShader(THIS_
	          ...
	          /* [annotation] */
	          __in_ecount_opt(NumClassInstances) ID3D11ClassInstance *const *ppClassInstances, UINT NumClassInstances,
	          std::set<UINT64> *visitedShaders, UINT64 selectedShader, UINT64 *currentShaderHash,
  ...
  public:
	...
	virtual void FrameAnalysisDump(ID3D11Resource *resource [[maybe_unused]],
	                               FrameAnalysisOptions options [[maybe_unused]],
	                               const wchar_t *target [[maybe_unused]], DXGI_FORMAT format [[maybe_unused]],
	...
	unsigned GetDispatchNumber() const
	{
