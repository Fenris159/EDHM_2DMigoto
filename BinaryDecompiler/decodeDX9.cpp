...
#include "log.h"
class DecompileErrorDX9 : public std::exception
{
} decompileErrorDX9; // 3DMigoto specific
...
static void DecodeOperandDX9(const Shader *psShader, const uint32_t ui32Token, const uint32_t ui32Token1,
                             uint32_t ui32Flags, Operand *psOperand)
{
...
static void DeclareNumTemps(Shader *psShader [[maybe_unused]], const uint32_t ui32NumTemps, Declaration *psDecl)
{
	psDecl->eOpcode = OPCODE_DCL_TEMPS;
	psDecl->value.ui32NumTemps = ui32NumTemps;
}

static void SetupRegisterUsage(const Shader *psShader [[maybe_unused]], const uint32_t ui32Token0,
...
{
	auto eUsage [[maybe_unused]] = (DECLUSAGE_DX9)0;
	...
	if (!psShader->sInfo->psConstantBuffers->asVars.empty())
	{
		...
		DecodeOperandDX9(psShader, CreateOperandTokenDX9(ui32RegNum, ui32RegType), 0, DX9_DECODE_OPERAND_IS_DECL,
		                 &psDecl->asOperands[0]);
		...
		psDecl->asOperands[0].aui32ArraySizes[1] =
		    psShader->sInfo->psConstantBuffers[0].ui32TotalSizeInBytes / 16; //Number of vec4 constants.
	}
}

static void DecodeDeclarationDX9(const Shader *psShader, const uint32_t ui32Token0, const uint32_t ui32Token1,
                                 Declaration *psDecl)
{
...
static void DefineDX9(Shader *psShader [[maybe_unused]], const uint32_t ui32RegNum, const uint32_t ui32Flags,
                      const uint32_t c0, const uint32_t c1, const uint32_t c2, const uint32_t c3, Declaration *psDecl)
{
	...
	psDecl->asOperands[1].iIntegerImmediate =
	    (ui32Flags & (DX9_DECODE_OPERAND_IS_ICONST | DX9_DECODE_OPERAND_IS_BCONST)) ? 1 : 0;
...
static void CreateD3D10Instruction(Shader *psShader, Instruction *psInst, const OPCODE_TYPE eType,
                                   const uint32_t bHasDest, const uint32_t ui32SrcCount, const uint32_t *pui32Tokens)
{
	uint32_t ui32Src;
	...
	*psInst = Instruction();
	...
	if (bHasDest)
	{
		...
		DecodeOperandDX9(psShader, pui32Tokens[ui32Offset], pui32Tokens[ui32Offset + 1], DX9_DECODE_OPERAND_IS_DEST,
		                 &psInst->asOperands[0]);
	...
	{
		DecodeOperandDX9(psShader, pui32Tokens[ui32Offset], pui32Tokens[ui32Offset + 1], DX9_DECODE_OPERAND_IS_SRC,
...
Shader *DecodeDX9BC(const uint32_t *pui32Tokens)
{
	...
	while (1)
	{
		...
		else if (eOpcode == OPCODE_DX9_DCL)
		{
			...
			if (psShader->eShaderType == PIXEL_SHADER &&
			    (OPERAND_TYPE_DX9_CONST != eType && OPERAND_TYPE_DX9_SAMPLER != eType))
			{
				ignoreDCL = 1;
			}
			if (!ignoreDCL)
	...
	while (1)
	{
		...
		else if ((eOpcode == OPCODE_DX9_DEF) || (eOpcode == OPCODE_DX9_DEFI) || (eOpcode == OPCODE_DX9_DEFB))
		{
			...
			DefineDX9(psShader, DecodeOperandRegisterNumberDX9(pui32CurrentToken[1]), ui32Flags, ui32Const0, ui32Const1,
			          ui32Const2, ui32Const3, &psDecl[decl]);
			decl++;
		}
		else
		{
