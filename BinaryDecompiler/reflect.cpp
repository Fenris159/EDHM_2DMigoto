...
static void ReadInputSignatures(const uint32_t *pui32Tokens, ShaderInfo *psShaderInfo, const int extended)
{
	...
	for (i = 0; i < ui32ElementCount; ++i)
	{
		...
		ReadStringFromTokenStream((const uint32_t *)((const char *)pui32FirstSignatureToken + ui32SemanticNameOffset),
		                          psCurrentSignature->SemanticName);
	}
}

static void ReadOutputSignatures(const uint32_t *pui32Tokens, ShaderInfo *psShaderInfo, const int minPrec,
                                 const int streams)
{
	uint32_t i;
...
static void ReadPatchConstantSignatures(const uint32_t *pui32Tokens, ShaderInfo *psShaderInfo, const int minPrec,
                                        const int streams)
{
	uint32_t i;
...
static const uint32_t *ReadResourceBinding(const uint32_t *pui32FirstResourceToken, const uint32_t *pui32Tokens,
                                           ResourceBinding *psBinding)
{
	uint32_t ui32NameOffset = *pui32Tokens++;

	ReadStringFromTokenStream((const uint32_t *)((const char *)pui32FirstResourceToken + ui32NameOffset),
	                          psBinding->Name);
...
static void ReadShaderVariableType(const uint32_t ui32MajorVersion, const uint32_t *pui32FirstConstBufToken,
                                   const uint32_t *pui32tokens, ShaderVarType *varType)
{
	const auto *pui16Tokens = (const uint16_t *)pui32tokens;
	...
	if (ui32MemberCount)
	{
		...
		for (i = 0; i < ui32MemberCount; ++i)
		{
			...
			ReadStringFromTokenStream((const uint32_t *)((const char *)pui32FirstConstBufToken + ui32NameOffset),
			                          varType->Members[i].Name);

			ReadShaderVariableType(ui32MajorVersion, pui32FirstConstBufToken,
			                       (const uint32_t *)((const char *)pui32FirstConstBufToken + ui32MemberTypeOffset),
			                       &varType->Members[i]);
		}
	}
}

static const uint32_t *ReadConstantBuffer(ShaderInfo *psShaderInfo, const uint32_t *pui32FirstConstBufToken,
                                          const uint32_t *pui32Tokens, ConstantBuffer *psBuffer)
{
	uint32_t i;
	uint32_t ui32NameOffset = *pui32Tokens++;
	uint32_t ui32VarCount = *pui32Tokens++;
	uint32_t ui32VarOffset = *pui32Tokens++;
	const auto *pui32VarToken = (const uint32_t *)((const char *)pui32FirstConstBufToken + ui32VarOffset);
	...
	for (i = 0; i < ui32VarCount; ++i)
	{
		...
		ReadShaderVariableType(psShaderInfo->ui32MajorVersion, pui32FirstConstBufToken,
		                       (const uint32_t *)((const char *)pui32FirstConstBufToken + ui32TypeOffset),
		                       &psVar->sType);
		...
		if (ui32DefaultValueOffset)
		{
			...
			const auto *pui32DefaultValToken =
			    (const uint32_t *)((const char *)pui32FirstConstBufToken + ui32DefaultValueOffset);
                          ...
                          ShaderInfo *psShaderInfo)    //out
{
	...
	uint32_t ui32CompileFlags [[maybe_unused]] =
	    *pui32Tokens++; //D3DCompile flags? http://msdn.microsoft.com/en-us/library/gg615083(v=vs.85).aspx
	...
	{
		pui32ConstantBuffers =
		    ReadConstantBuffer(psShaderInfo, pui32FirstToken, pui32ConstantBuffers, psConstantBuffers + i);
	}
...
static const uint16_t *ReadClassType(const uint32_t *pui32FirstInterfaceToken, const uint16_t *pui16Tokens,
                                     ClassType *psClassType)
{
	const auto *pui32Tokens = (const uint32_t *)pui16Tokens;
	uint32_t ui32NameOffset = *pui32Tokens;
	pui16Tokens += 2;
	...
	ReadStringFromTokenStream((const uint32_t *)((const char *)pui32FirstInterfaceToken + ui32NameOffset),
	                          psClassType->Name);

	return pui16Tokens;
}

static const uint16_t *ReadClassInstance(const uint32_t *pui32FirstInterfaceToken, const uint16_t *pui16Tokens,
                                         ClassInstance *psClassInstance)
{
...
static void ReadInterfaces(const uint32_t *pui32Tokens, ShaderInfo *psShaderInfo)
{
	...
	const auto *pui16ClassTypes = (const uint16_t *)((const char *)pui32FirstInterfaceToken + ui32ClassTypeOffset);
	const auto *pui16ClassInstances =
	    (const uint16_t *)((const char *)pui32FirstInterfaceToken + ui32ClassInstanceOffset);
	const auto *pui32InterfaceSlots =
	    (const uint32_t *)((const char *)pui32FirstInterfaceToken + ui32InterfaceSlotOffset);
	...
	for (i = 0; i < ui32InterfaceSlotRecordCount; ++i)
	{
		...
		const auto *pui16TypeID = (const uint16_t *)((const char *)pui32FirstInterfaceToken + ui32TypeIDOffset);
		const auto *pui32TableID = (const uint32_t *)((const char *)pui32FirstInterfaceToken + ui32TableIDOffset);
...
int GetResourceFromBindingPoint(const ResourceGroup eGroup, uint32_t const ui32BindPoint,
...
int GetInterfaceVarFromOffset(uint32_t ui32Offset, ShaderInfo *psShaderInfo, ShaderVar **ppsShaderVar)
{
	...
	const auto ui32NumVars = (uint32_t)psThisPointerConstBuffer->asVars.size();

	for (i = 0; i < ui32NumVars; ++i)
	{
		if (ui32Offset >= psThisPointerConstBuffer->asVars[i].ui32StartOffset &&
		    ui32Offset <
		        (psThisPointerConstBuffer->asVars[i].ui32StartOffset + psThisPointerConstBuffer->asVars[i].ui32Size))
		{
...
static int IsOffsetInType(ShaderVarType *psType, uint32_t parentOffset, uint32_t offsetToFind, int32_t *pi32Index,
                          int32_t *pi32Rebase)
{
	...
	if ((offsetToFind >= thisOffset) && offsetToFind < (thisOffset + thisSize))
	{
...
int GetShaderVarFromNestedStructOffset(ShaderVarType *psType, const uint32_t parentOffset, uint32_t ui32ByteOffset,
                                       ShaderVarType **ppsShaderVar, int32_t *pi32Index, int32_t *pi32Rebase)
{
	...
	if (ui32ByteOffset < thisOffset || ui32ByteOffset >= thisOffset + thisSize)
		return 0;
	...
	for (m = 0; m < psType->MemberCount; ++m)
	{
		...
		{
			if (GetShaderVarFromNestedStructOffset(psMember, thisOffset, ui32ByteOffset, ppsShaderVar, pi32Index,
			                                       pi32Rebase))
				return 1;
		}
		else
                           ...
                           ShaderVarType **ppsShaderVar, int32_t *pi32Index, int32_t *pi32Rebase)
{
	...
	const auto ui32NumVars = (uint32_t)psCBuf->asVars.size();
	...
	for (i = 0; i < ui32NumVars; ++i)
	{
		...
		{
			if (GetShaderVarFromNestedStructOffset(&psCBuf->asVars[i].sType, psCBuf->asVars[i].ui32StartOffset,
			                                       ui32ByteOffset, ppsShaderVar, pi32Index, pi32Rebase))
				return 1;
		}
		else
		{
			if (IsOffsetInType(&psCBuf->asVars[i].sType, psCBuf->asVars[i].ui32StartOffset, ui32ByteOffset, pi32Index,
			                   pi32Rebase))
			{
