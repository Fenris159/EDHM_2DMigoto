...
static struct format_type format_names[] = {
    {0, 0, "unknown"}, {1, 0, "uint"},   {1, 5, "min16u"}, // min16uint
    {2, 0, "int"},     {2, 4, "min16i"},                   // min16int *AND* min12int as of d3dcompiler47.dll
    {3, 0, "float"},   {3, 1, "min16f"},                   // min16float
...
static void *serialise_signature_section(const char *section24, const char *section28, const char *section32,
                                         int entry_size, vector<struct sgn_entry_unserialised> *entries,
                                         uint32_t name_len)
{
	void *section;
	...
	LogDebug("name_off: %u, name_len: %u, section_size: %u, padding: %u, alloc_size: %u\n", name_off, name_len,
	         section_size, padding, alloc_size);
	...
	LogDebug("section: 0x%p, section_header: 0x%p, sgn_header: 0x%p, padding_ptr: 0x%p, entry: 0x%p\n", section,
	         (char *)section_header, sgn_header, padding_ptr, entryn);
...
static void *parse_signature_section(const char *section24, const char *section28, const char *section32,
                                     string *shader, size_t *pos, bool invert_used, uint64_t sfi)
{
	string line;
	size_t old_pos = *pos;
	int numRead;
	...
	char mask[8];
	char used[8];    // We read 7 characters - check the reason below
	...
	while (*pos != shader->npos)
	{
		...
		if (line == "//" || line == "// Name                 Index   Mask Register SysValue  Format   Used" ||
		    line == "// -------------------- ----- ------ -------- -------- ------- ------")
		{
			continue;
		}
		...
		numRead = sscanf_s(
		    (line + "       ").c_str(), "// %s %d%7c %s %s %s%7c", semantic_name, (unsigned)ARRAYSIZE(semantic_name),
		    &entry.common.semantic_index, mask, (unsigned)ARRAYSIZE(mask), &reg, (unsigned)ARRAYSIZE(reg), system_value,
		    (unsigned)ARRAYSIZE(system_value), format, (unsigned)ARRAYSIZE(format), used, (unsigned)ARRAYSIZE(used));
	...
	name_already_used:
		...
		LogDebug("Stream: %i, Name: %s, Index: %i, Mask: 0x%x, Register: %i, SysValue: %i, Format: %i, Used: 0x%x, "
		         "Precision: %i\n",
		         entry.stream, entry.name.c_str(), entry.common.semantic_index, entry.common.mask, entry.common.reg,
		         entry.common.system_value, entry.common.format, entry.common.used, entry.min_precision);
...
static const char *subshader_feature_comments[] = {
    ...
    "UAVs at every shader stage", "64 UAV slots", "Minimum-precision data types",
    "Double-precision extensions for 11.1", "Shader extensions for 11.1", "Comparison filtering for feature level 9",
    // d3dcompiler_47:
    "Tiled resources", "PS Output Stencil Ref", "PS Inner Coverage", "Typed UAV Load Additional Formats",
    "Raster Ordered UAVs", "SV_RenderTargetArrayIndex or SV_ViewportArrayIndex from any shader feeding rasterizer",
    // DX12 DirectXShaderCompiler (tools/clang/tools/dxcompiler/dxcdisassembler.cpp)
    "Wave level operations", "64-Bit integer", "View Instancing", "Barycentrics", "Use native low precision",
    "Shading Rate"};
		...
		if (!strncmp(line.c_str(), "dcl_globalFlags ", 16))
		{
			...
			while (gf_pos != string::npos)
			{
				for (i = 0; i < ARRAYSIZE(global_flag_sfi_map); i++)
				{
					...
					{
						LogDebug("Mapped %s to Subshader Feature 0x%llx\n", global_flag_sfi_map[i].gf,
						         global_flag_sfi_map[i].sfi);
