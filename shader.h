...
struct sgn_entry_serialiased
{                         // Base version - 24 bytes
...
struct sg5_entry_serialiased
{ // Version "5" (only exists for geometry shader outputs) - 28 bytes
	uint32_t stream;
	struct sgn_entry_serialiased sgn;
};

struct sg1_entry_serialiased
{ // "Version "1" (most recent - I assume that's 5.1?) - 32 bytes
