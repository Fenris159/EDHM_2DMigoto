...
observed context plus a conservative `TextureOverride` candidate and its parent
`ShaderOverride`. The INI language cannot encode the complete context identity,
so the candidate is explicitly labeled for manual review and must retain the
reported parent-shader and binding constraint. The runtime does not modify
active configuration files automatically.
