...
enum DataType
{
	...
	DT_uint,
	DT_int,
	// Known limitation: Missing types added for primitive type StructuredBuffers, but not yet rest of decompiler:
	DT_float3x2,
	DT_float2x3,
	DT_float2x2,
	DT_float4x1,
	...
	DT_bool1x2,
	DT_bool1x1,
	// Known limitation: Add support for double, doubleN, doubleNxM (shader model 5+)
	// FUTURE: Minimum precision types (Win8+)
	DT_Unknown
};
struct BufferEntry
{
	DataType bt{DT_Unknown};
	int matrixRow{};
	bool isRowMajor{};
	string Name;
};
...
class Decompiler
{
  public:
	...
	Instruction *mLastStatement{nullptr};
...
#undef DT
	...
	bool SkipPacking(const char *c, map<string, DataType> inUse)
	{
		...
		int numRead = sscanf_s(c + pos, "// %s %d %s %d %s %s", name, UCOUNTOF(name), &index, mask, UCOUNTOF(mask),
		                       &reg1, sysvalue, UCOUNTOF(sysvalue), format, UCOUNTOF(format));
		if (numRead != 6)
			return false;
		...
		while (c[pos] != 0x0a && pos < 200)
			pos++;
		pos++;

		numRead = sscanf_s(c + pos, "// %s %d %s %d %s %s", name, UCOUNTOF(name), &index, mask, UCOUNTOF(mask), &reg2,
		                   sysvalue, UCOUNTOF(sysvalue), format, UCOUNTOF(format));
		if (numRead != 6)
			return false;
		name[sizeof(name) - 1] = '\0'; // Appease the static analysis gods
		if (strcmp(name, "TEXCOORD") != 0)
	...
	void ParseInputSignature(Shader *shader, const char *c, size_t size)
	{
		...
		while (pos < size)
		{
			...
			if (!strncmp(c + pos, "// no Input", strlen("// no Input")))
				break;
			int numRead = sscanf_s(c + pos, "// %s %d %s %d %s %s", name, UCOUNTOF(name), &index, mask, UCOUNTOF(mask),
			                       &slot, format2, UCOUNTOF(format2), format, UCOUNTOF(format));
			if (numRead != 6)
			{
			...
			if (SkipPacking(c + pos, usedInputRegisters))
				sprintf_s(format2, sizeof(format2), "%s%d", format, 4); // force to float4
	...
	void ParseOutputSignature(const char *c, size_t size)
	{
		...
		while (pos < size)
		{
			...
			else if (numRead == 3)
			{
				...
				numRead = sscanf_s(c + pos, "// %s %d %s %s %s %s", name, UCOUNTOF(name), &index, mask, UCOUNTOF(mask),
				                   reg, UCOUNTOF(reg), sysValue, UCOUNTOF(sysValue), format, UCOUNTOF(format));
	...
	void WriteZeroOutputSignature(const char *c, size_t size)
	{
		...
		while (pos < size)
		{
			...
			{
				char sysValue[64];
				int numRead [[maybe_unused]] =
				    sscanf_s(c + pos, "// %s %d %s %s %s %s", name, UCOUNTOF(name), &index, mask, UCOUNTOF(mask),
					         sysValue, UCOUNTOF(sysValue), format2, UCOUNTOF(format2), format, UCOUNTOF(format));
				// Write.
				char buffer[256];
				sprintf_s(buffer, sizeof(buffer), "  %s = 0;\n", sysValue);
				mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
	...
	void ReadResourceBindings(const char *c, size_t size)
	{
		...
		while (pos < size)
		{
			...
			int arraySize;
			type[0] = 0;
			int numRead = sscanf_s(c + pos, "// %s %s %s %s %s %d", name, UCOUNTOF(name), type, UCOUNTOF(type), format,
			                       UCOUNTOF(format), dim, UCOUNTOF(dim), bind, UCOUNTOF(bind), &arraySize);
		...
		for (map<int, string>::iterator i = mUAVNames.begin(); i != mUAVNames.end(); ++i)
		{
			...
			else if (mUAVNamesArraySize[i->first] > 1)
			{
				...
				sprintf_s(buffer, sizeof(buffer), "%s %s[%d] : register(u%d);\n", mUAVType[i->first].c_str(),
				          baseName.c_str(), mUAVNamesArraySize[i->first], i->first);
				mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
			}
		}
	}
		...
		while (pos < size - strlen(headerid))
		{
			...
			do
			{
				...
				if (!strcmp(type, "row_major") || !strcmp(type, "column_major"))
				{
					...
					modifier = type;
					modifier.push_back(' ');
					numRead = sscanf_s(c + pos, "// %s %s %[^;]", buffer, UCOUNTOF(buffer), type, UCOUNTOF(type), name,
					                   UCOUNTOF(name));
					if (numRead != 3)
					{
						logDecompileError("Error parsing buffer item: " + string(c + pos, 80));
				...
				*/
				...
				if (ep != string::npos &&
				    (e.bt == DT_bool || e.bt == DT_bool4 || e.bt == DT_float || e.bt == DT_float2 ||
				     e.bt == DT_float3 || e.bt == DT_float4 || e.bt == DT_uint || e.bt == DT_uint2 ||
				     e.bt == DT_uint3 || e.bt == DT_uint4 || e.bt == DT_int || e.bt == DT_int2 || e.bt == DT_int3 ||
				     ...
				     e.bt == DT_float3x3 || e.bt == DT_float4x2 || e.bt == DT_float2x4))
				{
					...
					if (e.bt == DT_float || e.bt == DT_bool || e.bt == DT_uint || e.bt == DT_int)
						...
						counter = 12;
					else if (e.bt == DT_bool4 || e.bt == DT_float4 || e.bt == DT_uint4 || e.bt == DT_int4)
						counter = 16;
				...
				else if (e.bt == DT_float4x2 || e.bt == DT_float3x3 || e.bt == DT_float4x3 || e.bt == DT_float2x4 ||
				         e.bt == DT_float3x4 || e.bt == DT_float4x4)
				{
					e.matrixRow = 0;
				...
				if (!strncmp(c + pos, defaultid, strlen(defaultid)))
				{
					...
					else if (e.bt == DT_bool)
					{
						...
						if (structLevel < 0)
						{
							...
							sprintf_s(buffer, sizeof(buffer), "  %s%s%s %s = %s;\n", structSpacing.c_str(),
							          modifier.c_str(), type, name, bString.c_str());
						mOutput.insert(mOutput.end(), buffer, buffer + strlen(buffer));
					}
					...
					else if (e.bt == DT_int || e.bt == DT_int2 || e.bt == DT_int3 || e.bt == DT_int4)
					{
						...
						{
							if (suboffset == 0)
								sprintf_s(buffer, sizeof(buffer), "  %s%s %s : packoffset(c%d) = {", modifier.c_str(),
								          type, name, packoffset);
							else
								sprintf_s(buffer, sizeof(buffer), "  %s%s %s : packoffset(c%d.%c) = {",
								          modifier.c_str(), type, name, packoffset, INDEX_MASK[suboffset]);
						}
						else
							sprintf_s(buffer, sizeof(buffer), "  %s%s%s %s = {", structSpacing.c_str(),
					...
					else if (e.bt == DT_float4x4)
					{
						...
						{
							numRead = sscanf_s(c + pos, "//%*[ =]0x%lx 0x%lx 0x%lx 0x%lx",
							                   (unsigned long *)&v[i * 4 + 0], (unsigned long *)&v[i * 4 + 1],
							                   (unsigned long *)&v[i * 4 + 2], (unsigned long *)&v[i * 4 + 3]);
							if (numRead != 4)
							{
					...
					{
						sprintf_s(buffer, sizeof(buffer), "  %s%s %s : packoffset(c%d) = {", modifier.c_str(), type,
						          name, packoffset);
						appendOutput(buffer);
				...
				else
				{
					if (structLevel < 0)
					{
						if (suboffset == 0)
							...
							sprintf_s(buffer, sizeof(buffer), "  %s%s %s : packoffset(c%d.%c);\n", modifier.c_str(),
							          type, name, packoffset, INDEX_MASK[suboffset]);
					}
					else
	...
	// Future work: Convert other parsers to use this helper
	...
	void applySwizzle(const char *left, char *right, bool useInt = false)
	{
		...
		else if (right[0] == 'c' && right[1] != 'b')
		{
			...
			// Known limitation: Refactor this into a dedicated function
			...
			if (strPos)
			{
				     ...
				     strrchr(right2, '.')[1] == 'w'))
				{
					...
					if (lowOffset >= skip)
					{
						...
						for (int cpos = 1; dotPos[cpos] >= 'w' && dotPos[cpos] <= 'z'; ++cpos)
						{
							...
							if (dotPos[cpos] == 'w')
								lowOffset = 3;
							const int remappedOffset = lowOffset - skipEnd;
							if (remappedOffset < 0 || remappedOffset >= 4)
							{
								logDecompileError("Error remapping buffer component: " + string(right2));
								return;
							}
							dotPos[cpos] = INDEX_MASK[remappedOffset];
						}
					}
				}
				...
				if (regAndSwiz[0])
				{
					...
					if (isCorrected != mCorrectedIndexRegisters.end())
					{
						...
						strcpy_s(newOperand, sizeof(newOperand), isCorrected->second.c_str());
						applySwizzle(regAndSwiz, newOperand, true);
						sprintf_s(right3 + strlen(right3), sizeof(right3) - strlen(right3), "[%s]", newOperand);
					}
					else if (
					    mLastStatement && mLastStatement->eOpcode == OPCODE_IMUL &&
					    (i->second.bt == DT_float4x4 || i->second.bt == DT_float4x3 || i->second.bt == DT_float4x2 ||
						 i->second.bt == DT_float2x4 || i->second.bt == DT_float3x4 || i->second.bt == DT_float3x3) &&
					    mMulOperand.substr(0, mMulOperand.find('.')) !=
					        indexRegisterName) // We can't use the old non-multiplied index variable if it was the same one overwritten with the result of the multiplication. TestShaders/GameExamples/DOAXVV/ba2ad61fa36ff709-vs.bin
					{
						char newOperand[opcodeSize];
						strcpy_s(newOperand, sizeof(newOperand), mMulOperand.c_str());
						applySwizzle(regAndSwiz, newOperand, true);
						sprintf_s(right3 + strlen(right3), sizeof(right3) - strlen(right3), "[%s]", newOperand);
					    ...
					    i->second.bt == DT_float2x4 || i->second.bt == DT_float3x4 || i->second.bt == DT_float3x3)
					{
						...
						while (*++strPos)
						{
							...
							case 'w':
								...
								break;
							default:
								logDecompileError("Error parsing matrix index: " + string(right2));
							}
						}
					}
					else
	...
	char statement[128]{}, op1[opcodeSize]{}, op2[opcodeSize]{}, op3[opcodeSize]{},
	    op4[opcodeSize]{}, // NOSONAR: fixed C buffers are required by the legacy parser APIs.
	...
	int ReadStatement(const char *pos)
	{
		...
		int numRead =
		    sscanf_s(lineBuffer, "%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s", statement, UCOUNTOF(statement), op1,
			         opcodeSize, op2, opcodeSize, op3, opcodeSize, op4, opcodeSize, op5, opcodeSize, op6, opcodeSize,
			         op7, opcodeSize, op8, opcodeSize, op9, opcodeSize, op10, opcodeSize, op11, opcodeSize, op12,
			         opcodeSize, op13, opcodeSize, op14, opcodeSize, op15, opcodeSize);
	...
	void convertHexToFloat(char *target)
	{
		...
		if (target[0] == 'l')
		{
			count = sscanf_s(target, "l(%x,%x,%x,%x)", (unsigned int *)&lit[0], (unsigned int *)&lit[1],
			                 (unsigned int *)&lit[2], (unsigned int *)&lit[3]);
			if (count != 0)
			{
	...
	char *_convertToInt(char *target, bool fixupCBs)
	{
		...
		if (!strncmp(target + isMinus, "float", 5))
		{
			...
			if (size == 2)
				...
				sprintf_s(buffer + strlen(buffer), opcodeSize - strlen(buffer), "int3(%d,%d,%d)", (int)f0, (int)f1,
				          (int)f2);
			else if (size == 4)
				sprintf_s(buffer + strlen(buffer), opcodeSize - strlen(buffer), "int4(%d,%d,%d,%d)", (int)f0, (int)f1,
				          (int)f2, (int)f3);
	...
	char *_convertToUInt(char *target, bool fixupCBs)
	{
		...
		if (!strncmp(target + isMinus, "float", 5))
		{
			...
			if (size == 2)
				...
				sprintf_s(buffer + strlen(buffer), opcodeSize - strlen(buffer), "uint3(%d,%d,%d)", (int)f0, (int)f1,
				          (int)f2);
			else if (size == 4)
				sprintf_s(buffer + strlen(buffer), opcodeSize - strlen(buffer), "uint4(%d,%d,%d,%d)", (int)f0, (int)f1,
				          (int)f2, (int)f3);
		...
		if (o.eType == OPERAND_TYPE_IMMEDIATE32)
		{
			...
			if (!strncmp(op, "l(1.#INF00", strlen("l(1.#INF00")) || abs(oldValue - o.afImmediates[0]) < 0.1)
			{
				if (o.iNumComponents == 4)
					sprintf_s(op, opcodeSize, "l(%.9g,%.9g,%.9g,%.9g)", o.afImmediates[0], o.afImmediates[1],
					          o.afImmediates[2], o.afImmediates[3]);
				else if (o.iNumComponents == 3)
					sprintf_s(op, opcodeSize, "l(%.9g,%.9g,%.9g)", o.afImmediates[0], o.afImmediates[1],
					          o.afImmediates[2]);
		...
		if (!mShaderType.substr(0, 3).compare("ps_"))
		{
			...
			if (depthTexture != mTextureNames.end())
			{
				...
				if (G->ZRepair_Dependencies1.empty() || found == (1 << G->ZRepair_Dependencies1.size()) - 1)
				{
					...
					sprintf_s(op1, sizeof(op1), " = %s.Sample", G->ZRepair_DepthTexture1.c_str());
					...
					while (pos)
					{
						...
						if (!wposAvailable)
						{
							sprintf_s(buf, sizeof(buf),
							          ...
							          "float zpos = %s;\n"
							          "float wpos = 1.0 / zpos;\n",
							          depthBufferStatement.c_str(), G->ZRepair_DepthTextureReg1,
							          G->ZRepair_ZPosCalc1.c_str());
						}
						else
						{
							sprintf_s(buf, sizeof(buf),
			...
			if (!wposAvailable)
			{
				...
				if (depthTexture != mTextureNames.end())
				{
					...
					if (G->ZRepair_Dependencies2.empty() || found == (1 << G->ZRepair_Dependencies2.size()) - 1)
					{
						mOutput.push_back(0);
						// Search depth texture usage.
						sprintf_s(op1, sizeof(op1), " = %s.Sample", G->ZRepair_DepthTexture2.c_str());
						char *pos = strstr(mOutput.data(), op1);
						if (pos)
						{
							char *bpos = pos;
							...
							sprintf_s(buf, sizeof(buf),
							          ...
							          "float zpos = %s;\n"
							          "float wpos = 1.0 / zpos;\n",
							          depthBufferStatement.c_str(), G->ZRepair_DepthTextureReg2,
							          G->ZRepair_ZPosCalc2.c_str());
			...
			if (!wposAvailable)
			{
				...
				if (positionTexture != mTextureNames.end())
				{
					...
					sprintf_s(op1, sizeof(op1), " = %s.Sample", G->ZRepair_PositionTexture.c_str());
					char *pos = strstr(mOutput.data(), op1);
					if (pos)
					{
						char *bpos = pos;
					...
					if (!G->ObjectPos_ID1.empty())
					{
						...
						while (offset < mOutput.size())
						{
							    ...
							    (pos[3] == 'x' || pos[3] == 'y' || pos[3] == 'z'))
							{
								...
								if (size >= sizeof(op1))
								{
									logDecompileError("Object position expression is too long");
									return;
								}
								...
								sprintf_s(
								    ...
								    uuidVar, G->ObjectPos_MUL1.c_str(), op2, lightPosDecl[0], uuidVar, op3,
						...
						while (offset < mOutput.size())
						{
							    ...
							    (pos[3] == 'x' || pos[3] == 'y' || pos[3] == 'z'))
							{
								...
								sprintf_s(
								    ...
								    "\n%s += viewDirection.%c * separation * (wpos - convergence) * stereoPos%dMul.y;"
								    "\n%s += viewDirection.%c * separation * (wpos - convergence) * stereoPos%dMul.z;",
								    uuidVar, G->ObjectPos_MUL2.c_str(), op2, spotPosDecl[0], uuidVar, op3,
					...
					if (!G->MatrixPos_ID1.empty())
					{
						...
						if (pos1 && pos2 && pos3)
						{
							...
							_snprintf_s(
							    ...
							    uuidVar, G->MatrixPos_MUL1.c_str(), regName1.c_str(), uuidVar, regName2.c_str(),
	...
	void CreateRawFormat(string texType, int bufIndex)
	{
		...
		if (sscanf_s(op1, "(%[^,]", format, 16) != 1)
		{ // Match first xx of (xx,xx,xx,xx)
	...
	bool translate_structured_var(Shader *shader, const char *c, size_t &pos, size_t &size, Instruction *instr,
	                              std::string ret[4], bool *combined, char *idx, char *off, char *reg, Operand *texture,
	                              int swiz_offsets[4])
	{
		...
		*combined = false;
		...
		if (group != (ResourceGroup)-1 &&
		    GetResourceFromBindingPoint(group, texture->ui32RegisterNumber, shader->sInfo, &bindInfo))
		{
			...
			if (mStructuredBufferUsedNames.find(struct_type_i->second) != mStructuredBufferUsedNames.end())
			{
				...
				if (sscanf_s(off, "%d", &swiz_offset) == 1)
				{
					...
					for (uint32_t component = 0; component < 4; component++)
					{
						...
						sprintf_s(buffer, sizeof(buffer), "%s[%s].%s", bindInfo->Name.c_str(), ci(idx).c_str(),
						          var_txt.c_str());
			...
			else
			{
				...
				if (sscanf_s(off, "%d", &swiz_offset) == 1)
				{
					...
					sprintf_s(buffer, sizeof(buffer), "%s[%s].%s%s%s%s", bindInfo->Name.c_str(), ci(idx).c_str(),
				...
				else
				{
					...
					sprintf_s(buffer, sizeof(buffer), "%s[%s][%s/4]", bindInfo->Name.c_str(), ci(idx).c_str(),
					          ci(off).c_str());
				}
		...
		else
		{
			...
			// based on the swizzle. Future work: We could recombine them using
			...
			for (int component = 0; component < 4; component++)
			{
				...
				sprintf_s(buffer, sizeof(buffer), "%s[%s].val[%s/4%s]", reg, ci(idx).c_str(), ci(off).c_str(),
				          swiz_offset);
				ret[component] = buffer;
			}
			return true;
	...
	void parse_ld_structured(Shader *shader, const char *c, size_t &pos, size_t &size, Instruction *instr)
	{
		...
		std::array<int, 4> swiz_offsets{0, 4, 8, 12};
		for (int component = 0; component < 4; component++)
		{
			switch (texture.aui32Swizzle[component])
			{
			case OPERAND_4_COMPONENT_X:
				swiz_offsets[component] = 0;
				break;
			case OPERAND_4_COMPONENT_Y:
				swiz_offsets[component] = 4;
				break;
			case OPERAND_4_COMPONENT_Z:
				swiz_offsets[component] = 8;
				break;
			case OPERAND_4_COMPONENT_W:
				swiz_offsets[component] = 12;
				break;
			}
		}

		if (translate_structured_var(shader, c, pos, size, instr, translated.data(), &combined, idx, off, reg, &texture,
		                             swiz_offsets.data()))
		{
			if (combined)
			{
			...
			else
			{
				...
				for (int component = 0; component < 4; component++)
				{
					...
					sprintf_s(buffer, sizeof(buffer), "  %s.%c = %s;\n", writeTarget(dst),
					          component == 3 ? 'w' : 'x' + component, translated[component].c_str());
					appendOutput(buffer);
				}
			}
		}
	...
	void parse_store_structured(Shader *shader, const char *c, size_t &pos, size_t &size, Instruction *instr)
	{
		...
		if (translate_structured_var(shader, c, pos, size, instr, translated.data(), &combined, idx, off, dst, &dst0,
		                             swiz_offsets.data()))
		{
			if (combined)
			{
	...
	int IsInstructionOperandSame(Instruction *instr1, int opIndex1, Instruction *instr2, int opIndex2,
	                             const char *instr1Op1 = nullptr, const char *instr2Op1 = nullptr)
	{
		...
		if (same)
		{
			if (((op1.eModifier == OPERAND_MODIFIER_NEG || op1.eModifier == OPERAND_MODIFIER_ABSNEG) &&
			     (op2.eModifier == OPERAND_MODIFIER_NONE || op2.eModifier == OPERAND_MODIFIER_ABS)) ||
			    ((op2.eModifier == OPERAND_MODIFIER_NEG || op2.eModifier == OPERAND_MODIFIER_ABSNEG) &&
			     (op1.eModifier == OPERAND_MODIFIER_NONE || op1.eModifier == OPERAND_MODIFIER_ABS)))
			{
				return 2;
			}
	...
	void ParseCode(Shader *shader, const char *c, size_t size)
	{
		...
		while (pos < size && iNr < inst_count)
		{
			...
			// avoid crashes.
			if (!shader->dx9Shader && (!strncmp(statement, "vs_1", 4) || !strncmp(statement, "vs_2", 4) ||
			...
			if (!strncmp(statement, "vs_", 3) || !strncmp(statement, "hs_", 3) || !strncmp(statement, "ds_", 3) ||
			    !strncmp(statement, "gs_", 3) || !strncmp(statement, "ps_", 3) || !strncmp(statement, "cs_", 3))
			{
				skip_shader = false;
			...
			else if (!strcmp(statement, "dcl_resource_structured") || !strcmp(statement, "dcl_uav_structured"))
			{
				...
				if (mStructuredBufferTypes.empty())
				{
					sprintf_s(buffer, sizeof(buffer),
					          ...
					          prefix, bufIndex, bufStride / 4, uav ? "RW" : "", prefix, bufIndex, prefix, bufIndex,
					          prefix, bufIndex);
					mOutput.insert(mOutput.begin(), buffer, buffer + strlen(buffer));
					mCodeStartPos += strlen(buffer);
					...
					if (bufStride % 4)
					{
						...
						sprintf_s(buffer, sizeof(buffer),
						          "Known limitation: StructuredBuffer t%d stride %d is not a multiple of 4\n\n",
						          bufIndex, bufStride);
						mOutput.insert(mOutput.begin(), buffer, buffer + strlen(buffer));
						mCodeStartPos += strlen(buffer);
					}
			...
			// there is no Resource Binding section in the shader.  Future work: probably needs to handle arrays too.
				...
				if (op2[0] == 't')
				{
					...
					if (i == mTextureNames.end())
					{
						CreateRawFormat("Texture2D", bufIndex);
			                 ...
			                 "dcl_resource_texture2darray")) // dcl_resource_texture2darray (float,float,float,float) t0
			{
				if (op2[0] == 't')
				{
					int bufIndex = 0;
					...
					if (i == mTextureNames.end())
					{
						CreateRawFormat("Texture2DArray", bufIndex);
			             ...
			             statement, "dcl_resource_texture2dms",
			             strlen(
			                 "dcl_resource_texture2dms"))) // dcl_resource_texture2dms(8) (float,float,float,float) t4
			{
				if (op2[0] == 't')
				{
					int bufIndex = 0;
			...
			// For Geometry Shaders, e.g. dcl_stream m0  Future work: make it StreamN, add to varlist
			...
			else
			{
				...
				case OPCODE_UBFE:
				{
					...
					*pop1 = 0;
					while (*++pop1)
					{
						sprintf_s(op5, sizeof(op5), "%s.%c", op1, *pop1);
						sprintf_s(buffer, sizeof(buffer), "  if (%s == 0) %s = 0; else if (%s+%s < 32) { ",
						          ci(GetSuffix(op2, idx)).c_str(), writeTarget(op5), ci(GetSuffix(op2, idx)).c_str(),
						          ci(GetSuffix(op3, idx)).c_str());
						appendOutput(buffer);
						// Known limitation: May need fixup for read from constant buffer of unidentified type?
						sprintf_s(buffer, sizeof(buffer), "%s = (uint)%s << (32-(%s + %s)); %s = (uint)%s >> (32-%s); ",
						          writeTarget(op5), ci(GetSuffix(op4, idx)).c_str(), ci(GetSuffix(op2, idx)).c_str(),
						          ci(GetSuffix(op3, idx)).c_str(), writeTarget(op5), writeTarget(op5),
						          ci(GetSuffix(op2, idx)).c_str());
						appendOutput(buffer);
				...
				case OPCODE_IBFE:
				{
					...
					sprintf_s(buffer, sizeof(buffer),
					          ...
					          "(int%s)%s >> %s"
					          ")));\n",
					          writeTarget(op1), ci(op2).c_str(), ci(op2).c_str(), ci(op3).c_str(),
					          swizCount(op4).c_str(), ci(op4).c_str(), ci(op2).c_str(), ci(op3).c_str(),
					          ci(op2).c_str(), swizCount(op4).c_str(), ci(op4).c_str(), ci(op3).c_str());
				...
				case OPCODE_LOG:
				{
					...
					if (shader->dx9Shader &&
					    (iNr + lookahead <
					     inst_count)) // Known limitation: This is actually a lookahead code path and may not be DX9 specific
					{
				...
				case OPCODE_UMIN:
					...
					applySwizzle(op1, fixImm(op3, instr->asOperands[2]));
					sprintf_s(buffer, sizeof(buffer), "  %s = min(%s, %s);\n", writeTarget(op1),
					          ci(convertToUInt(op3)).c_str(), ci(convertToUInt(op2)).c_str());
					appendOutput(buffer);
					removeBoolean(op1);
					break;
				...
				case OPCODE_IMM_ATOMIC_CMP_EXCH:
				{
					...
					ASMLineOut(c, pos, size);
					sprintf_s(buffer, sizeof(buffer),
					          "  InterlockedCompareExchange(dest, compare_value, imm_value, orig_value);\n");
					appendOutput(buffer);
					break;
				}
				...
				case OPCODE_MAX:
				{
					     ...
					     inst_count)) // Known limitation: This is actually a lookahead code path and may not be DX9 specific
					{
						...
						if (nextIns->eOpcode == OPCODE_MAD &&
						    ...
						    IsInstructionOperandSame(instr, 0, nextIns, 2, nullptr,
				...
				case OPCODE_IMIN:
					...
					if (!instr->bSaturate)
						sprintf_s(buffer, sizeof(buffer), "  %s = min(%s, %s);\n", writeTarget(op1),
						          ci(convertToInt(op3)).c_str(), ci(convertToInt(op2)).c_str());
					else
						sprintf_s(buffer, sizeof(buffer), "  %s = saturate(min(%s, %s));\n", writeTarget(op1),
						          ci(convertToInt(op3)).c_str(), ci(convertToInt(op2)).c_str());
					appendOutput(buffer);
				...
				case OPCODE_MAD:
					...
					/*
						...
						const size_t op4Offset = (op4[0] == '-') ? 1u : 0u;
						sprintf_s(op4 + op4Offset, sizeof(op4) - op4Offset, "(%s * %s)", mMulOperand.c_str(), mMulOperand2.c_str());
						*/
					if (!instr->bSaturate)
						sprintf_s(buffer, sizeof(buffer), "  %s = %s * %s + %s;\n", writeTarget(op1), ci(op2).c_str(),
				...
				case OPCODE_DP3:
				{
					     ...
					     inst_count)) // Known limitation: This is actually a lookahead code path and may not be DX9 specific
					{
						...
						if (nextIns[0]->eOpcode == OPCODE_ADD && nextIns[1]->eOpcode == OPCODE_MAD &&
						    ...
						    IsInstructionOperandSame(instr, 1, nextIns[1], 3, nullptr, outputOp1.c_str()) == 1 &&
						    IsInstructionOperandSame(instr, 2, nextIns[1], 1, nullptr, outputOp1.c_str()) == 1)
						{
							//read next instruction
							for (int i = 0; i < lookahead; i++)
							{
				...
				case OPCODE_SWAPC:
				{
					...
					*pop1 = 0;
					...
					while (*++pop1)
					{
						sprintf_s(op6, sizeof(op6), "%s.%c", op1, *pop1);
						if (pop2)
							sprintf_s(op7, sizeof(op7), "%s.%c", op2, *++pop2);
						else
							sprintf_s(op7, sizeof(op7), "%s", op2);
						if (pop3)
							sprintf_s(op8, sizeof(op8), "%s.%c", op3, *++pop3);
						else
							sprintf_s(op8, sizeof(op8), "%s", op3);
						// Known limitation: May need fixup for read from constant buffer of unidentified type
					...
					while (*++pop1)
					{
						...
						sprintf_s(buffer, sizeof(buffer),
						          ...
						          *pop1, ci(GetSuffix(op2, idx)).c_str(), ci(GetSuffix(op3, idx)).c_str(),
						          writeTarget(op6), ci(GetSuffix(op4, idx)).c_str(), ci(GetSuffix(op3, idx)).c_str(),
						          *pop1, ci(GetSuffix(op5, idx)).c_str(), *pop1);
						appendOutput(buffer);
						++idx;
					}
					break;
				...
				case OPCODE_SAMPLE_B:
				{
					...
					else
					{
						...
						sprintf_s(buffer, sizeof(buffer), "  %s = %s.SampleBias(%s, %s, %s, int2(%d, %d))%s;\n",
						          writeTarget(op1), mTextureNames[textureId].c_str(), mSamplerNames[samplerId].c_str(),
						          ci(op2).c_str(), ci(op5).c_str(), offsetx, offsety, strrchr(op3, '.'));
					}
					appendOutput(buffer);
					removeBoolean(op1);
					break;
				...
				case OPCODE_SAMPLE_D:
				{
					...
					if (!instr->bAddressOffset)
						sprintf_s(buffer, sizeof(buffer), "  %s = %s.SampleGrad(%s, %s, %s, %s)%s;\n", writeTarget(op1),
						          mTextureNames[textureId].c_str(), mSamplerNames[samplerId].c_str(), ci(op2).c_str(),
						          ci(op5).c_str(), ci(op6).c_str(), strrchr(op3, '.'));
					else
					{
						int offsetx = 0, offsety = 0, offsetz = 0;
						...
						sprintf_s(buffer, sizeof(buffer), "  %s = %s.SampleGrad(%s, %s, %s, %s, int2(%d, %d))%s;\n",
						          writeTarget(op1), mTextureNames[textureId].c_str(), mSamplerNames[samplerId].c_str(),
						          ci(op2).c_str(), ci(op5).c_str(), ci(op6).c_str(), offsetx, offsety,
						          strrchr(op3, '.'));
					}
					appendOutput(buffer);
					removeBoolean(op1);
				...
				case OPCODE_SAMPLE_C:
				{
					...
					else
					{
						...
						sprintf_s(buffer, sizeof(buffer), "  %s = %s.SampleCmp(%s, %s, %s, int2(%d, %d))%s;\n",
						          writeTarget(op1), mTextureNames[textureId].c_str(),
						          mSamplerComparisonNames[samplerId].c_str(), ci(op2).c_str(), ci(op5).c_str(), offsetx,
						          offsety, strrchr(op3, '.'));
					}
					appendOutput(buffer);
					removeBoolean(op1);
				...
				case OPCODE_SAMPLE_C_LZ:
				{
					...
					if (!instr->bAddressOffset)
						sprintf_s(buffer, sizeof(buffer), "  %s = %s.SampleCmpLevelZero(%s, %s, %s)%s;\n",
						          writeTarget(op1), mTextureNames[textureId].c_str(),
						          mSamplerComparisonNames[samplerId].c_str(), ci(op2).c_str(), ci(op5).c_str(),
						          strrchr(op3, '.'));
					else
					{
						int offsetx = 0, offsety = 0, offsetz = 0;
				...
				case OPCODE_LD:
				{
					...
					else
					{
						...
						sprintf_s(buffer, sizeof(buffer), "  %s = %s.Load(%s, int3(%d, %d, %d))%s;\n", writeTarget(op1),
						          mTextureNames[textureId].c_str(), ci(op2).c_str(), offsetU, offsetV, offsetW,
						          strrchr(op3, '.'));
					}
					appendOutput(buffer);
					removeBoolean(op1);
				...
				case OPCODE_RESINFO:
				{
					...
					if (bindStripped)
					{
						...
						int numInfo =
						    sscanf_s(statement, "resinfo_indexable(%[^)])%s", texType, opcodeSize, retType, opcodeSize);
					...
					if ((constZero.eType == OPERAND_TYPE_IMMEDIATE32) && (constZero.afImmediates[0] == 0.0) &&
					    (returnType == RESINFO_INSTRUCTION_RETURN_UINT ||
					     returnType == RESINFO_INSTRUCTION_RETURN_FLOAT) &&
					    texture.eType == OPERAND_TYPE_RESOURCE && !bindStripped)
					{
						...
						if (bindInfoPtr->eDimension == REFLECT_RESOURCE_DIMENSION_TEXTURE2D)
						{
							if (returnType == RESINFO_INSTRUCTION_RETURN_UINT)
								sprintf_s(buffer, sizeof(buffer),
								          "  %s.GetDimensions(0, uiDest.x, uiDest.y, uiDest.z);\n",
					...
					if (unknownVariant)
					{
						...
						sprintf_s(buffer, sizeof(buffer),
						          "// Unknown use of GetDimensions for resinfo_ from missing reflection info, need "
						          "manual fix.\n");
						appendOutput(buffer);
						sprintf_s(buffer, sizeof(buffer), "// %s\n", line.c_str());
						appendOutput(buffer);
						...
						sprintf_s(
						    ...
						    " state=%d, constZero.eType=%d, returnType=%d, texture.eType=%d, afImmediates[0]=%f\n",
	...
	void WriteAddOnDeclarations()
	{
		...
		declaration += "#define cmp -\n";

		if (G->IniParamsReg >= 0)
		{
			declaration += "Texture1D<float4> IniParams : register(t" + std::to_string(G->IniParamsReg) + ");\n";
		}

		if (G->StereoParamsReg >= 0)
		{
			declaration += "Texture2D<float4> StereoParams : register(t" + std::to_string(G->StereoParamsReg) + ");\n";
		}

		if (G->ZRepair_DepthBuffer)
		{
			declaration += "Texture2D<float4> InjectedDepthTexture : register(t126);\n";
		}
	...
	{
		string header = "// ---- Created with 3Dmigoto v" + string(VER_FILE_VERSION_STR) + " on " + LogTime();
